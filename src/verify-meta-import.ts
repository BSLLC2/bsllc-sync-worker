...
import {
  ...
  countMetaConversions, metaAccountId, metaConversionWindow, metaLinkClicks,
  metaMonthKey, metaMonthlyMetrics, metaSpendToMicros, monthsBackStart,
...
console.log("1. Spend is a decimal string in the account's currency, stored as micros");
{
  ...
  const m = metaMonthlyMetrics(AUGUST, metaConversionWindow([AUGUST]));
...
console.log("\n2. CTR is a FRACTION here and a PERCENTAGE at Meta");
{
  ...
  const noImpressions = metaMonthlyMetrics({ ...AUGUST, impressions: "0" }, [0]);
...
console.log("\n3. A click is a LINK click, and a substitution is named");
{
  ...
  const noClicks = metaMonthlyMetrics({ ...AUGUST, clicks: "0", inline_link_clicks: "0" }, [0]);
...
console.log("\n5. A conversion nought is only a nought where the pixel has ever fired");
{
  ...
  const window = metaConversionWindow(configured);
  ...
  const blindWindow = metaConversionWindow(neverAny);
...
console.log("\n6. Nothing served is no_data, not a row of noughts");
{
  ...
  const r = metaMonthlyMetrics(empty, [0]);
  ...
  const served = metaMonthlyMetrics(AUGUST, metaConversionWindow([AUGUST]));
...
console.log("\n7. The key set is the declared one");
{
  ...
  ok("the monthly field list asks for what the rules read",
    ["spend", "impressions", "clicks", "inline_link_clicks", "actions"].every((f) => META_MONTHLY_FIELDS.split(",").includes(f)));
